...
The transition dynamics being followed is according to Vinh(1981). Initially 3dof model is being studied and upon successful completion 6dof model will be applied.
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, PlanetModel=Planet('Earth'), render_mode: Optional[str] = None):
        ...
        self.final_state = np.array([6408.0e3, 1.0, 0.0, 1600.0, 0.0, 0.0], dtype=np.float32)
        ...
        if self.steps_beyond_terminated is None:
            ...
            if not terminated:
                ...
                if np.all(abs(self.state - self.final_state) <= np.array([0.1, 0.01, 0.01, 0.1, 0.01, 0.01])):
                    ...
                    reward += 2000
                elif np.all(abs(self.state - self.final_state) <= np.array([8000.0, 3.5, 1.5, 2000.0, 1.5, 1.5])):
                    reward += 1500
                elif np.all(abs(self.state - self.final_state) <= np.array([10.0e3, 4.0, 2.0, 2200.0, 2.0, 2.0])):
                    reward += 1250
                elif np.all(abs(self.state - self.final_state) <= np.array([12.0e3, 4.0, 2.0, 2400.0, 2.0, 2.0])):
                    reward += 1000
                elif np.all(abs(self.state - self.final_state) <= np.array([14.0e3, 4.0, 2.0, 2600.0, 2.0, 2.0])):
                    reward += 900
                elif np.all(abs(self.state - self.final_state) <= np.array([16.0e3, 4.0, 2.0, 2800.0, 2.0, 2.0])):
                    reward += 800
                elif np.all(abs(self.state - self.final_state) <= np.array([18.0e3, 5.0, 2.0, 3000.0, 2.0, 2.0])):
                    reward += 700
                elif np.all(abs(self.state - self.final_state) <= np.array([20.0e3, 5.0, 2.0, 3200.0, 2.0, 2.0])):
                    reward += 600
                elif np.all(abs(self.state - self.final_state) <= np.array([22.0e3, 5.0, 2.0, 3400.0, 2.0, 2.0])):
                    reward += 500
                elif np.all(abs(self.state - self.final_state) <= np.array([23.0e3, 5.0, 2.0, 3600.0, 2.0, 2.0])):
                    reward += 400
                elif np.all(abs(self.state - self.final_state) <= np.array([24.0e3, 5.0, 2.0, 3800.0, 2.0, 2.0])):
                    reward += 300
                elif np.all(abs(self.state - self.final_state) <= np.array([25.5e3, 5.0, 2.0, 4000.0, 2.0, 2.0])):
                    reward += 250
                elif np.all(abs(self.state - self.final_state) <= np.array([26.0e3, 6.3, 2.0, 4200.0, 2.0, 2.0])):
                    reward += 225
                elif np.all(abs(self.state - self.final_state) <= np.array([27.0e3, 6.3, 2.0, 4250.0, 2.0, 2.0])):
                    reward += 200
                elif np.all(abs(self.state - self.final_state) <= np.array([28.0e3, 6.3, 2.0, 4300.0, 2.0, 2.0])):
                    reward += 175
                elif np.all(abs(self.state - self.final_state) <= np.array([29.0e3, 6.3, 2.0, 4350.0, 2.0, 2.0])):
                    reward += 150
                elif np.all(abs(self.state - self.final_state) <= np.array([30.0e3, 6.3, 2.0, 4400.0, 2.0, 2.0])):
                    reward += 130
                elif np.all(abs(self.state - self.final_state) <= np.array([31.0e3, 6.3, 2.0, 4425.0, 2.0, 2.0])):
                    reward += 120
                elif np.all(abs(self.state - self.final_state) <= np.array([32.0e3, 6.3, 2.0, 4450.0, 2.0, 2.0])):
                    reward += 110
                elif np.all(abs(self.state - self.final_state) <= np.array([33.0e3, 6.3, 2.0, 4475.0, 2.0, 2.0])):
                    reward += 100
                elif np.all(abs(self.state - self.final_state) <= np.array([34.0e3, 6.3, 2.0, 4500.0, 2.0, 2.0])):
                    reward += 90
                elif np.all(abs(self.state - self.final_state) <= np.array([35.0e3, 6.3, 2.0, 4525.0, 2.0, 2.0])):
                    reward += 80
                elif np.all(abs(self.state - self.final_state) <= np.array([36.0e3, 6.3, 2.0, 4550.0, 2.0, 2.0])):
                    reward += 70
                elif np.all(abs(self.state - self.final_state) <= np.array([37.0e3, 6.3, 2.0, 4575.0, 2.0, 2.0])):
                    reward += 60
                elif np.all(abs(self.state - self.final_state) <= np.array([38.0e3, 6.3, 2.0, 4600.0, 2.0, 2.0])):
                    reward += 55
                elif np.all(abs(self.state - self.final_state) <= np.array([39.0e3, 6.3, 2.0, 4625.0, 2.0, 2.0])):
                    reward += 50
                elif np.all(abs(self.state - self.final_state) <= np.array([40.0e3, 6.3, 2.0, 4650.0, 2.0, 2.0])):
                    reward += 45
                elif np.all(abs(self.state - self.final_state) <= np.array([41.5e3, 6.3, 2.0, 4675.0, 2.0, 2.0])):
                    reward += 40
                elif np.all(abs(self.state - self.final_state) <= np.array([42.5e3, 6.3, 2.0, 4700.0, 2.0, 2.0])):
                    reward += 35
                elif np.all(abs(self.state - self.final_state) <= np.array([43.5e3, 6.3, 2.0, 4725.0, 2.0, 2.0])):
                    reward += 30
                elif np.all(abs(self.state - self.final_state) <= np.array([44.5e3, 6.3, 2.0, 4750.0, 2.0, 2.0])):
                    reward += 25
                elif np.all(abs(self.state - self.final_state) <= np.array([45.5e3, 6.3, 2.0, 4775.0, 2.0, 2.0])):
                    reward += 20
                elif np.all(abs(self.state - self.final_state) <= np.array([46.5e3, 6.3, 2.0, 4800.0, 2.0, 2.0])):
                    reward += 15
                elif np.all(abs(self.state - self.final_state) <= np.array([47.5e3, 6.3, 2.0, 4825.0, 2.0, 2.0])):
                    reward += 10
                elif np.all(abs(self.state - self.final_state) <= np.array([48.5e3, 6.3, 2.0, 4850.0, 2.0, 2.0])):
                    reward += 5
                elif np.all(abs(self.state - self.final_state) <= np.array([49.5e3, 6.3, 2.0, 4875.0, 2.0, 2.0])):
                    reward += 1.5
                elif np.all(abs(self.state - self.final_state) <= np.array([50.0e3, 6.3, 2.0, 4900.0, 2.0, 2.0])):
                    reward += 1
